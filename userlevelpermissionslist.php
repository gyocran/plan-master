...
&nbsp;&nbsp;<a href="<?php echo $userlevelpermissions_list->ExportCsvUrl ?>"><?php echo $Language->Phrase("ExportToCsv") ?></a>
<?php } ?>
</span></p>
...
<table class="ewBasicSearch">
	<tr>
		<td><span class="phpmaker">
			...
			<a href="userlevelpermissionssrch.php"><?php echo $Language->Phrase("AdvancedSearch") ?></a>&nbsp;
		</span></td>
	</tr>
</table>
</form>
...
class cuserlevelpermissions_list {
	...
	function Page_Init() {
		...
		if ($userlevelpermissions->Export == "csv") {
	...
	function AdvancedSearchWhere() {
		...
		$this->BuildSearchSql($sWhere, $userlevelpermissions->userlevelid, FALSE); // userlevelid
		$this->BuildSearchSql($sWhere, $userlevelpermissions->ztablename, FALSE); // tablename
		$this->BuildSearchSql($sWhere, $userlevelpermissions->permission, FALSE); // permission

		// Set up search parm
		if ($sWhere <> "") {
			$this->SetSearchParm($userlevelpermissions->userlevelid); // userlevelid
			$this->SetSearchParm($userlevelpermissions->ztablename); // tablename
			$this->SetSearchParm($userlevelpermissions->permission); // permission
		}
		return $sWhere;
	}
	...
	function GetSearchParm(&$Fld) {
		global $userlevelpermissions;
		$FldParm = substr($Fld->FldVar, 2);
		$Fld->AdvancedSearch->SearchValue = $userlevelpermissions->GetAdvancedSearch("x_$FldParm");
		$Fld->AdvancedSearch->SearchOperator = $userlevelpermissions->GetAdvancedSearch("z_$FldParm");
		$Fld->AdvancedSearch->SearchCondition = $userlevelpermissions->GetAdvancedSearch("v_$FldParm");
		$Fld->AdvancedSearch->SearchValue2 = $userlevelpermissions->GetAdvancedSearch("y_$FldParm");
		$Fld->AdvancedSearch->SearchOperator2 = $userlevelpermissions->GetAdvancedSearch("w_$FldParm");
	}
	...
	function ResetAdvancedSearchParms() {
		...
		$userlevelpermissions->setAdvancedSearch("x_ztablename", "");
		$userlevelpermissions->setAdvancedSearch("x_permission", "");
	}
	...
	function RestoreSearchParms() {
		...
		if (@$_GET["x_ztablename"] <> "") $bRestore = FALSE;
		if (@$_GET["x_permission"] <> "") $bRestore = FALSE;
		$this->RestoreSearch = $bRestore;
		if ($bRestore) {

			// Restore advanced search values
			$this->GetSearchParm($userlevelpermissions->userlevelid);
			$this->GetSearchParm($userlevelpermissions->ztablename);
			$this->GetSearchParm($userlevelpermissions->permission);
		}
	}
	...
	function LoadSearchValues() {
		...
		$userlevelpermissions->userlevelid->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_userlevelid"]);
		$userlevelpermissions->userlevelid->AdvancedSearch->SearchOperator = @$_GET["z_userlevelid"];

		// tablename
		$userlevelpermissions->ztablename->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_ztablename"]);
		$userlevelpermissions->ztablename->AdvancedSearch->SearchOperator = @$_GET["z_ztablename"];

		// permission
		$userlevelpermissions->permission->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_permission"]);
		$userlevelpermissions->permission->AdvancedSearch->SearchOperator = @$_GET["z_permission"];
	}
