...
if (is_array($arwrk)) {
	$armultiwrk= explode(",", strval($userlevelpermissions->permission->CurrentValue));
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
...
class cuserlevelpermissions_add {
	...
	function ValidateForm() {
		...
		if ($userlevelpermissions->permission->FormValue == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
	...
	function AddRow() {
		...
		if ($userlevelpermissions->ztablename->CurrentValue == "" && $userlevelpermissions->ztablename->getSessionValue() == "") {
