...
// Create page object
$school_attendance_add = new cschool_attendance_add();
$Page =& $school_attendance_add;

// Page init
$school_attendance_add->Page_Init();

// Page main
$school_attendance_add->Page_Main();
?>
...
// Create page object
var school_attendance_add = new ew_Page("school_attendance_add");

// page properties
school_attendance_add.PageID = "add"; // page ID
school_attendance_add.FormID = "fschool_attendanceadd"; // form ID
var EW_PAGE_ID = school_attendance_add.PageID; // for backward compatibility

// extend page with ValidateForm function
school_attendance_add.ValidateForm = function(fobj) {
...
school_attendance_add.Form_CustomValidate =  
...
school_attendance_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
school_attendance_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $school_attendance->TableCaption() ?><br><br>
<a href="<?php echo $school_attendance->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$school_attendance_add->ShowMessage();
?>
<form name="fschool_attendanceadd" id="fschool_attendanceadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return school_attendance_add.ValidateForm(this);">
<p>
...
<?php
$school_attendance_add->Page_Terminate();
?>
<?php
...
class cschool_attendance_add {
	...
	// Page object name
	var $PageObjName = 'school_attendance_add';
	...
	//
	function cschool_attendance_add() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $school_attendance->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$school_attendance->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $school_attendance->CurrentAction = "C"; // Copy record
		  } else {
		    $school_attendance->CurrentAction = "I"; // Display blank record
		...
		switch ($school_attendance->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$school_attendance->SendEmail = TRUE; // Send email on add success
		...
		$school_attendance->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $school_attendance;
		$school_attendance->group_id->CurrentValue = CurrentUserID();
	}
	...
	function RenderRow() {
		...
		} elseif ($school_attendance->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			$bValidUser = $Security->IsValidUserID($school_attendance->group_id->CurrentValue);
			if (!$bValidUser) {
				$sUserIdMsg = str_replace("%c", CurrentUserID(), $Language->Phrase("UnAuthorizedUserID"));
				$sUserIdMsg = str_replace("%u", $school_attendance->group_id->CurrentValue, $sUserIdMsg);
		...
		if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			$sFilter = $school_attendance->SqlMasterFilter_sponsored_student();
			if (strval($school_attendance->sponsored_student_sponsored_student_id->CurrentValue) <> "" &&
				$school_attendance->getCurrentMasterTable() == "sponsored_student") {
				$sFilter = str_replace("@sponsored_student_id@", ew_AdjustSql($school_attendance->sponsored_student_sponsored_student_id->CurrentValue), $sFilter);
			} else {
				$sFilter = "";
			}
			if ($sFilter <> "") {			
				$rsmaster = $GLOBALS["sponsored_student"]->LoadRs($sFilter);
			...
			$sFilter = $school_attendance->SqlMasterFilter_sponsored_student_detail();
			if (strval($school_attendance->sponsored_student_sponsored_student_id->CurrentValue) <> "" &&
				$school_attendance->getCurrentMasterTable() == "sponsored_student_detail") {
				$sFilter = str_replace("@sponsored_student_id@", ew_AdjustSql($school_attendance->sponsored_student_sponsored_student_id->CurrentValue), $sFilter);
			} else {
				$sFilter = "";
			}
			if ($sFilter <> "") {			
				$rsmaster = $GLOBALS["sponsored_student_detail"]->LoadRs($sFilter);
				$this->bMasterRecordExists = ($rsmaster && !$rsmaster->EOF);
				if (!$this->bMasterRecordExists) {
					$sMasterUserIdMsg = str_replace("%c", CurrentUserID(), $Language->Phrase("UnAuthorizedMasterUserID"));
					$sMasterUserIdMsg = str_replace("%f", $sFilter, $sMasterUserIdMsg);
		...
		$bInsertRow = $school_attendance->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($school_attendance->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$school_attendance->school_attendance_id->setDbValue($conn->Insert_ID());
			$rsnew['school_attendance_id'] = $school_attendance->school_attendance_id->DbValue;

			// Call Row Inserted event
			$school_attendance->Row_Inserted($rsnew);
