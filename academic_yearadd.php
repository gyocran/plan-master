...
// Create page object
$academic_year_add = new cacademic_year_add();
$Page =& $academic_year_add;

// Page init
$academic_year_add->Page_Init();

// Page main
$academic_year_add->Page_Main();
?>
...
// Create page object
var academic_year_add = new ew_Page("academic_year_add");

// page properties
academic_year_add.PageID = "add"; // page ID
academic_year_add.FormID = "facademic_yearadd"; // form ID
var EW_PAGE_ID = academic_year_add.PageID; // for backward compatibility

// extend page with ValidateForm function
academic_year_add.ValidateForm = function(fobj) {
...
academic_year_add.Form_CustomValidate =  
...
academic_year_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
academic_year_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $academic_year->TableCaption() ?><br><br>
<a href="<?php echo $academic_year->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$academic_year_add->ShowMessage();
?>
<form name="facademic_yearadd" id="facademic_yearadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return academic_year_add.ValidateForm(this);">
<p>
...
<input type="text" name="x_app_year" id="x_app_year" title="<?php echo $academic_year->app_year->FldTitle() ?>" size="30" value="<?php echo $academic_year->app_year->EditValue ?>"<?php echo $academic_year->app_year->EditAttributes() ?>>
...
<?php
$academic_year_add->Page_Terminate();
?>
<?php
...
class cacademic_year_add {
	...
	// Page object name
	var $PageObjName = 'academic_year_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $academic_year->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$academic_year->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $academic_year->CurrentAction = "C"; // Copy record
		  } else {
		    $academic_year->CurrentAction = "I"; // Display blank record
		...
		switch ($academic_year->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$academic_year->SendEmail = TRUE; // Send email on add success
		...
		$academic_year->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($academic_year->RowType == EW_ROWTYPE_ADD) { // Add row

			// app_year
			$academic_year->app_year->EditCustomAttributes = "";
			$academic_year->app_year->EditValue = ew_HtmlEncode($academic_year->app_year->CurrentValue);
	...
	function AddRow() {
		...
		if ($academic_year->app_year->CurrentValue == "" && $academic_year->app_year->getSessionValue() == "") {
		...
		$sFilter = $academic_year->KeyFilter();
		if ($bCheckKey) {
			$rsChk = $academic_year->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
		...
		// app_year
		$academic_year->app_year->SetDbValueDef($rsnew, $academic_year->app_year->CurrentValue, 0, FALSE);

		// active
		$academic_year->active->SetDbValueDef($rsnew, $academic_year->active->CurrentValue, "", FALSE);

		// Call Row Inserting event
		$bInsertRow = $academic_year->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($academic_year->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			...
			$academic_year->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
