...
// Create page object
$grade_subject_edit = new cgrade_subject_edit();
$Page =& $grade_subject_edit;

// Page init
$grade_subject_edit->Page_Init();

// Page main
$grade_subject_edit->Page_Main();
?>
...
// Create page object
var grade_subject_edit = new ew_Page("grade_subject_edit");

// page properties
grade_subject_edit.PageID = "edit"; // page ID
grade_subject_edit.FormID = "fgrade_subjectedit"; // form ID
var EW_PAGE_ID = grade_subject_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
grade_subject_edit.ValidateForm = function(fobj) {
...
grade_subject_edit.Form_CustomValidate =  
...
grade_subject_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
grade_subject_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $grade_subject->TableCaption() ?><br><br>
<a href="<?php echo $grade_subject->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$grade_subject_edit->ShowMessage();
?>
<form name="fgrade_subjectedit" id="fgrade_subjectedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return grade_subject_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="grade_subject">
...
<?php if ($grade_subject->grade_subject_id->Visible) { // grade_subject_id ?>
	<tr<?php echo $grade_subject->RowAttributes() ?>>
		...
		<td<?php echo $grade_subject->grade_subject_id->CellAttributes() ?>><span id="el_grade_subject_id">
<div<?php echo $grade_subject->grade_subject_id->ViewAttributes() ?>><?php echo $grade_subject->grade_subject_id->EditValue ?></div><input type="hidden" name="x_grade_subject_id" id="x_grade_subject_id" value="<?php echo ew_HtmlEncode($grade_subject->grade_subject_id->CurrentValue) ?>">
</span><?php echo $grade_subject->grade_subject_id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php
$grade_subject_edit->Page_Terminate();
?>
<?php
...
class cgrade_subject_edit {
	...
	// Page object name
	var $PageObjName = 'grade_subject_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$grade_subject->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$grade_subject->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$grade_subject->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$grade_subject->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($grade_subject->grade_subject_id->CurrentValue == "")
			$this->Page_Terminate("grade_subjectlist.php"); // Invalid key, return to list
		switch ($grade_subject->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$grade_subject->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$grade_subject->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($grade_subject->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// grade_subject_id
			$grade_subject->grade_subject_id->EditCustomAttributes = "";
			$grade_subject->grade_subject_id->EditValue = $grade_subject->grade_subject_id->CurrentValue;
			$grade_subject->grade_subject_id->CssStyle = "";
			$grade_subject->grade_subject_id->CssClass = "";
			$grade_subject->grade_subject_id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $grade_subject->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($grade_subject->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$grade_subject->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
