...
//
// Table class for grade_year
//
class cgrade_year {
	var $TableVar = 'grade_year';
	...
	//
	function cgrade_year() {
		global $Language;

		// grade_year_id
		$this->grade_year_id = new cField('grade_year', 'grade_year', 'x_grade_year_id', 'grade_year_id', '`grade_year_id`', 3, -1, FALSE, '`grade_year_id`', FALSE);
		$this->grade_year_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['grade_year_id'] =& $this->grade_year_id;

		// class
		$this->class = new cField('grade_year', 'grade_year', 'x_class', 'class', '`class`', 200, -1, FALSE, '`class`', FALSE);
		$this->fields['class'] =& $this->class;

		// year
		$this->year = new cField('grade_year', 'grade_year', 'x_year', 'year', '`year`', 3, -1, FALSE, '`year`', FALSE);
		$this->year->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['year'] =& $this->year;

		// promoted
		$this->promoted = new cField('grade_year', 'grade_year', 'x_promoted', 'promoted', '`promoted`', 16, -1, FALSE, '`promoted`', FALSE);
		$this->promoted->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['promoted'] =& $this->promoted;

		// programme
		$this->programme = new cField('grade_year', 'grade_year', 'x_programme', 'programme', '`programme`', 200, -1, FALSE, '`programme`', FALSE);
		$this->fields['programme'] =& $this->programme;

		// school_attendance_school_attendance_id
		$this->school_attendance_school_attendance_id = new cField('grade_year', 'grade_year', 'x_school_attendance_school_attendance_id', 'school_attendance_school_attendance_id', '`school_attendance_school_attendance_id`', 3, -1, FALSE, '`school_attendance_school_attendance_id`', FALSE);
	...
	// Master filter
	function SqlMasterFilter_school_attendance() {
		return "`school_attendance_id`=@school_attendance_id@";
	}

	// Detail filter
	function SqlDetailFilter_school_attendance() {
		return "`school_attendance_school_attendance_id`=@school_attendance_school_attendance_id@";
	}
	...
	function ApplyUserIDFilters($sFilter) {
		...
		if (!$this->AllowAnonymousUser() && $Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			if ($this->getCurrentMasterTable() == "school_attendance")
				$sFilter = $this->AddDetailUserIDFilter($sFilter, "school_attendance"); // Add detail User ID filter
		}
		return $sFilter;
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `grade_year` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `grade_year` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `grade_year` WHERE ";
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("grade_yearview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "grade_yearadd.php";
	...
	// Edit URL
	function EditUrl() {
		return $this->KeyUrl("grade_yearedit.php", $this->UrlParm());
	}
	...
	// Copy URL
	function CopyUrl() {
		return $this->KeyUrl("grade_yearadd.php", $this->UrlParm());
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("grade_yeardelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=grade_year" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
   ...
   // Common render codes
		...
		if (strval($this->promoted->CurrentValue) <> "") {
			switch ($this->promoted->CurrentValue) {
				case "1":
					$this->promoted->ViewValue = "Yes";
					break;
				case "0":
					$this->promoted->ViewValue = "No";
					break;
				default:
	...
	function AddMasterUserIDFilter($sFilter, $sCurrentMasterTable) {
		$sFilterWrk = $sFilter;
		if ($sCurrentMasterTable == "school_attendance") {
			$sFilterWrk = $GLOBALS["school_attendance"]->AddUserIDFilter($sFilterWrk);
		}
		return $sFilterWrk;
	}
	...
	function AddDetailUserIDFilter($sFilter, $sCurrentMasterTable) {
		$sFilterWrk = $sFilter;
		if ($sCurrentMasterTable == "school_attendance") {
			$sSubqueryWrk = $GLOBALS["school_attendance"]->GetUserIDSubquery($this->school_attendance_school_attendance_id, $GLOBALS["school_attendance"]->school_attendance_id);
			if ($sSubqueryWrk <> "") {
				if ($sFilterWrk <> "") {
