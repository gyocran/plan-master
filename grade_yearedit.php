...
// Create page object
$grade_year_edit = new cgrade_year_edit();
$Page =& $grade_year_edit;

// Page init
$grade_year_edit->Page_Init();

// Page main
$grade_year_edit->Page_Main();
?>
...
// Create page object
var grade_year_edit = new ew_Page("grade_year_edit");

// page properties
grade_year_edit.PageID = "edit"; // page ID
grade_year_edit.FormID = "fgrade_yearedit"; // form ID
var EW_PAGE_ID = grade_year_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
grade_year_edit.ValidateForm = function(fobj) {
...
grade_year_edit.Form_CustomValidate =  
...
grade_year_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
grade_year_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $grade_year->TableCaption() ?><br><br>
<a href="<?php echo $grade_year->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$grade_year_edit->ShowMessage();
?>
<form name="fgrade_yearedit" id="fgrade_yearedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return grade_year_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="grade_year">
...
<input type="hidden" name="x_grade_year_id" id="x_grade_year_id" value="<?php echo ew_HtmlEncode($grade_year->grade_year_id->CurrentValue) ?>">
<p>
...
<?php
$grade_year_edit->Page_Terminate();
?>
<?php
...
class cgrade_year_edit {
	...
	// Page object name
	var $PageObjName = 'grade_year_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$grade_year->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$grade_year->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$grade_year->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$grade_year->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($grade_year->grade_year_id->CurrentValue == "")
			$this->Page_Terminate("grade_yearlist.php"); // Invalid key, return to list
		switch ($grade_year->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$grade_year->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$grade_year->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($grade_year->RowType == EW_ROWTYPE_EDIT) { // Edit row
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $grade_year->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($grade_year->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$grade_year->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
